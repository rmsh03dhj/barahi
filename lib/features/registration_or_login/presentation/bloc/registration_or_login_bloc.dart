...
class RegistrationOrLoginBloc
    extends Bloc<RegistrationOrLoginEvent, RegistrationOrLoginState> {
  ...
  @override
  Stream<RegistrationOrLoginState> mapEventToState(
      RegistrationOrLoginEvent event) async* {
    if (event is SignUpPressed) {
      ...
      yield failureOrUser
          .fold((failure) => SignUpFailedState(failure.failureMessage), (user) {
    ...
    if (event is SignInPressed) {
      ...
      final failureOrUser = await signInUseCase
          .execute(SignInParams(password: event.password, email: event.email));
      yield failureOrUser
          .fold((failure) => SignInFailedState(failure.failureMessage), (user) {
